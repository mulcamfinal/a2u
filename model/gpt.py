...
class GPT2SelfAttentionLayer(HybridBlock):
    ...
    def __init__(self, units, num_heads, dropout=0.0,
        ...
        super(GPT2SelfAttentionLayer, self).__init__(
    ...
    def hybrid_forward(self, F, data, states=None):  # pylint: disable=arguments-differ
        ...
        if states is not None:
            ...
            data_pos = F.broadcast_add(
                F.contrib.arange_like(data, axis=1), prev_len)
            all_pos = F.contrib.arange_like(
                F.concat(prev_len_range, data_len_range, dim=0))
        else:
        ...
        mask = F.broadcast_lesser_equal(
            all_pos.reshape((1, -1)), data_pos.reshape((-1, 1)))
        mask = F.broadcast_like(F.expand_dims(
            mask, axis=0), data, lhs_axes=(0, ), rhs_axes=(0, ))
        mask = F.concat(*[mask] * self._num_heads, dim=0)

        # Multi-head attention
        # Shape (batch_size, seq_len, 3 * units)
        qkv = self._multi_head_qkv_proj(data)
        ...
        if prev_key is not None:
            ...
            key = F.concat(prev_key.reshape(
                (-1, 0, 0), reverse=True), key, dim=1)
...
class GPT2Model(HybridBlock):
    ...
    def hybrid_forward(self, F, data, states=None):  # pylint: disable=arguments-differ
        ...
        new_states = []
        if states is not None:
            prev_len_range = F.contrib.arange_like(
                states[0], axis=2).astype('int32')
            prev_len = F.broadcast_add(F.slice_axis(prev_len_range, axis=0, begin=-1, end=None),
        ...
        if F is mx.nd:
            length = data.shape[1] + \
                (states[0].shape[2] if states is not None else 0)
